...
// 1. Extract Card
$cardPattern = '/\{\{-- ── ESCOLHA O TIPO DE MONITORAMENTO \(CARD\) ─────────────────────────── --\}\}.*?\{\{-- Conteúdo do card em branco ficará aqui --\}\}\s*<\/div>/s';
if (preg_match($cardPattern, $idxContent, $matchesCard)) {
    $cardHtml = $matchesCard[0];
...
// 5. Add required styles for the modal overlay/dialog (borrowed from index) if monitoramentos lacks them, but the modal uses inline styles and classes that we can duplicate or just copy over the basic css.
$cssSnippet = <<<CSS
    @push('styles')
        <style>
            .lf-esc-overlay {
