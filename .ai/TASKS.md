...
| `SKILLS-UPD-002` | Governança / Tooling | DONE | OpenCode | - | Instalar 3 skills UI v17.3.0 (`laravel-expert`, `frontend-dev-guidelines`, `redesign-existing-projects`) em `.agents/skills/`. Catalogação e versionamento concluídos; smoke test na IDE verificado. Lock `.ai/locks/SKILLS-UPD-002.lock.yaml` RELEASED. |
| `DOCKER-003` | Infra / Build | DONE | Antigravity | - | Bump v3.56.1 (Chatwoot Lead Chat modal, proxy controller e triggers EAV) + build/push `suitezap/lawfirm:3.56.1`, `suitezap/lawfirm:v3.56.1` e `latest` (digest `sha256:0401da4e36bf9cc833304a088a13e733a355d3146fb473ac1dd83e7d7f75d7e0`) no Docker Hub com validação de higiene estrita. |
| `WA-TPL-001` | Whatsapp / MotherShip | VERIFIED | Antigravity | - | Controle de Templates WhatsApp pelo MotherShip: tabela `lawfirm_whatsapp_templates` no mothership_db + model `MothershipWhatsappTemplate` + hierarquia de resolução 3 camadas no `WhatsappTemplatesController` + seed 14 templates + `ARCHITECTURE_mothership_orient.md` §21 atualizado. |
| `WA-TPL-002` | Whatsapp / UI | DONE | Antigravity | - | Redesign da interface de edição de templates em `/admin/juridico/whatsapp/templates`. Layout em grid de cards com preview e badge de customização. Modal flutuante para edição com suporte a injeção de variáveis com um clique e `window.__LF_WA_TPL_MAP` sem dados em eventos inline (cumprindo SKILL.md §6). |
