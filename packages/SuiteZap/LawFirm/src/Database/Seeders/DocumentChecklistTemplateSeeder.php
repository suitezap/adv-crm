...
/**
 * Seed da tabela law_checklist_templates com os Kits de Documentos padrão.
 *
 * IDEMPOTENTE: Usa updateOrCreate pelo campo 'name'.
 * Pode ser rodado múltiplas vezes sem duplicar registros.
 *
 * Para executar manualmente em um Tenant:
 *   php artisan db:seed --class="SuiteZap\LawFirm\Database\Seeders\DocumentChecklistTemplateSeeder"
 */
class DocumentChecklistTemplateSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($kits as $kit) {
            $result = ChecklistTemplate::updateOrCreate(
                ['name' => $kit['name']],
                [
                    'area'  => $kit['area'],
                    'items' => $kit['items'],
        ...
        $this->command->info("✅ DocumentChecklistTemplateSeeder: {$created} kit(s) criado(s), {$updated} atualizado(s).");
    }
}
